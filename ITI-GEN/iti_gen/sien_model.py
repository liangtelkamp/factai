...
class ITI_GEN(object):
    ...
    def setup_clip_model(self):
        ...
        self.args.device = "cuda".format(self.args.device) if torch.cuda.is_available() else "cpu" # Liang
        print(f"Device used for training the model {self.args.device}")
    ...
    def iti_gen_loss(self, image_features, text_features, cate_num, each_index, data_cls):
        ...
        for comb_idx, (first_class, second_class) in enumerate(all_comb):
            ...
            print(f'shape ori text feature = {self.ori_text_feature.shape}')
            print(f' 0 ori = {self.ori_text_feature[0]}')
            print(f' 1 ori = {self.ori_text_feature[1]}')
            print(f'shape text features = {text_features.shape}')
            logits_con = self.ori_text_feature @ text_features.t()  # 108*108

            print(f'shape logits con = {logits_con.shape}')
            print(f' logit values = {logits_con}')
